...
<template>
  ...
  >
    <button
      type="button"
      @click="selectPlace"
      class="w-full text-left outline-none rounded-xl focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-surface"
      ...
      <div class="flex items-start gap-3.5">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between gap-2">
            <span class="font-semibold text-base text-text-primary truncate group-hover:text-primary transition-colors">
              {{ place.name }}
            </span>
          ...
          <!-- Tags Row -->
          <div v-if="place.tags && place.tags.length > 0" class="flex flex-wrap gap-1 mt-2" :class="{ 'pr-20': place.status === 'ranked' }">
            <span
    ...
    <button
      ...
      class="absolute bottom-4 right-4 shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all active:scale-95"
